...
void LoadDatabase()
{
    ...
    if (SD2Database.Initialize(strSD2DBinfo.c_str()))
    {
        ...
        pSystemMgr.LoadScriptWaypoints();
			//wuzhu start
			pSystemMgr.LoadWUZHU_texts();
			pSystemMgr.LoadGameTele_Map();
			pSystemMgr.LoadTeleport_Template_Map();
			pSystemMgr.LoadWUZHU_Fly_Data();
			//wuzhu end
    }
    else
    {
...
bool GossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    ...
    pPlayer->PlayerTalkClass->ClearMenus();//wuzhu
...
{
	if(!pPlayer||!_Item) return false;
	Script *tmpscript = m_scripts->at(_Item->GetProto()->ScriptId);
	if(!tmpscript || !tmpscript->pGossipItemSelect) return false;
